...
\begin{figure}[!ht]
\textbf{FAZER:} colocar a figura do esquema de projeção de lepetit
\caption{Esquema de projeção. Retirado de \cite{lepetit}}
...
\begin{figure}[!ht]
	\centerline{
		...
		\subfloat[Este objeto não possui textura, porém suas arestas são bem definidas. Imagem retirada de \cite{marchand_modelbased}]{
			\shortstack{\textbf{FAZER:} Porca do artigo de\\marchand sobre model-based (1999)}
...
\begin{figure}[!ht]
\shortstack{\textbf{FAZER:} pegar um cubo do pipeline de \cite{drummondecipolla}\\que mostra um cubo sólido com um wireframe tentando se encaixar}
